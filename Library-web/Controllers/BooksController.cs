...
namespace Library_web.Controllers
{
    public class BooksController : Controller
    {
        ...
        {
            var client = httpClientFactory.CreateClient();
            var httpResponseMess1 = await client.GetAsync($"https://localhost:7245/api/Authors/get-all-author");
            httpResponseMess1.EnsureSuccessStatusCode();
            ViewBag.ListAuthor = await httpResponseMess1.Content.ReadFromJsonAsync<List<authorDTO>>();

            var httpResponseMess2 = await client.GetAsync($"https://localhost:7245/api/Publishers/get-all-publishers");
            httpResponseMess2.EnsureSuccessStatusCode();
            ViewBag.ListPublisher = await httpResponseMess2.Content.ReadFromJsonAsync<List<publisherDTO>>();
        ...
        public async Task<IActionResult> addBook(addBookDTO addBookDTO)
        {
            try
            {
                ...
                var client = httpClientFactory.CreateClient();
                var url = "https://localhost:7245/api/Books/add-book";

                // Gửi JSON đúng kiểu
                var res = await client.PostAsJsonAsync(url, addBookDTO);

                if (!res.IsSuccessStatusCode)
                {
                    var body = await res.Content.ReadAsStringAsync(); 
                    ViewBag.Error = $"API {(int)res.StatusCode} - {res.ReasonPhrase}\n{body}";
        ...
        public async Task<IActionResult> editBook(int id)
        {
            ...
            var httpResponsePu = await client.GetAsync("https://localhost:7245/api/Publishers/get-all-publishers");
